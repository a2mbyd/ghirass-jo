...
const Create = ({ sections, allCourses, allMajors }: CreateProps) => {
    ...
    const addMajorAssignment = () => {
        ...
        form.setData('majors', [
            ...form.data.majors,
            {
                ...
                course_major_type: newMajorForm.type,
            },
        ]);
        setNewMajorForm({
            id: '',
    ...
    const addFile = () => {
        ...
        form.setData('files', [...form.data.files, { ...newFileForm }]);
    ...
    const addVideo = () => {
        ...
        form.setData('videos', [...form.data.videos, { ...newVideoForm }]);
    ...
    const removeVideo = (index: number) => {
        form.setData(
            'videos',
            form.data.videos.filter((_, i) => i !== index),
        );
    };
    ...
    const addPYQ = () => {
        ...
        form.setData('past_year_questions', [
    ...
    const removePYQ = (index: number) => {
        form.setData(
            'past_year_questions',
            form.data.past_year_questions.filter((_, i) => i !== index),
        );
    };
    ...
    return (
        <div className="min-h-screen bg-background p-6 md:p-8" dir="rtl">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-text-muted">
                            أنشئ مادة دراسية جديدة وأضف تفاصيلها
                        </p>
                    </div>
                </div>
                <Link
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* ── Card 2: Majors ─────────────────────────────────── */}
                {!form.data.type.endsWith('university') && (
                            ...
                            {showNewMajorForm && (
                                <div className="rounded-xl border border-primary-200 bg-primary-50/40 p-4">
                                    ...
                                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
                                        ...
                                        {form.data.type === 'major_course' && (
                                            <select
                                                value={newMajorForm.type}
                                                onChange={(e) =>
                                                    setNewMajorForm((f) => ({
                                                        ...f,
                                                        type: e.target.value,
                ...
                <section className="animate-fade-in-up stagger-5 overflow-hidden rounded-2xl border border-border bg-surface shadow-(--shadow-soft)">
                    ...
                    <div className="space-y-4 p-6">
                        ...
                        {form.data.videos.length === 0 ? (
                            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-surface-alt py-12">
                                <Video className="h-9 w-9 text-text-subtle" />
                                <p className="mt-2 text-sm text-text-subtle">
                                    لم تتم إضافة مقاطع فيديو بعد
                                </p>
                            </div>
                        ) : (
                            <div className="overflow-hidden rounded-xl border border-border">
                                <div className="divide-y divide-border">
                                    {form.data.videos.map((video, idx) => (
                                        <PendingResourceRow
                                            key={`v-${idx}`}
                                            title={video.title}
                                            url={video.url}
                ...
                <section className="animate-fade-in-up stagger-5 overflow-hidden rounded-2xl border border-border bg-surface shadow-(--shadow-soft)">
                    <div className="flex items-center justify-between border-b border-border px-6 py-4">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="rounded-full bg-surface-alt px-2.5 py-0.5 text-xs font-medium text-text-muted">
                                {form.data.past_year_questions.length} ملف
                            </span>
                        </div>
                        <button
                            type="button"
                    ...
                    <div className="space-y-4 p-6">
                        ...
                        {form.data.past_year_questions.length === 0 ? (
                            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-surface-alt py-12">
                                <ClipboardList className="h-9 w-9 text-text-subtle" />
                                <p className="mt-2 text-sm text-text-subtle">
                                    لم تتم إضافة أسئلة سنوات سابقة بعد
                                </p>
                            </div>
                        ) : (
                            <div className="overflow-hidden rounded-xl border border-border">
                                <div className="divide-y divide-border">
                                    {form.data.past_year_questions.map(
                                        (pyq, idx) => (
                                            <div
                                                key={`pyq-${idx}`}
