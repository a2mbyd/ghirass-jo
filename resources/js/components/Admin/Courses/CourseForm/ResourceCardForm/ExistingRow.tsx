...
const ExistingRow = ({
    ...
    return (
        ...
        >
            {item.badge ? (
                <span
                    className={`inline-flex shrink-0 items-center rounded-lg border px-2.5 py-1 text-xs font-semibold ${item.badge.className} ${isDeleted ? 'line-through' : ''}`}
                >
                    {item.badge.text}
                </span>
            ) : (
            ...
            <div className="min-w-0 flex-1">
                {!item.badge && (
                    <p
                        className={`truncate text-sm font-medium text-text ${isDeleted ? 'line-through' : ''}`}
                    >
                        {item.label}
                    </p>
                )}
